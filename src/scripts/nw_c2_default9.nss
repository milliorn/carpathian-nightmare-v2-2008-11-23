...
void main()
{
    ...
    */
    ...
    WalkWayPoints();
    ExecuteScript("acn_voiceset",OBJECT_SELF);
...
*/
// EDIT BY CHEETAH: If we have 'SU_' at the beginning of our tag, then we're a
//                  summon, so don't despawn.
...
string sLeft = GetStringLeft(sTag, 3);
    ...
    DelayCommand(2.0, RandomNameNPC(OBJECT_SELF));
//    DelayCommand(2.0, ExecuteScript("acn_npc_despawn", OBJECT_SELF));
    DelayCommand(2.0, ActionCastSpellAtObject(SPELL_SEE_INVISIBILITY, OBJECT_SELF, METAMAGIC_MAXIMIZE, TRUE, 40, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
    ...
    if ( d6() == 6)
    {
        ...
        if (d3() == 1)
        {
            DoTreasureGeneration( OBJECT_SELF, OBJECT_SELF, LOW, GOLD|DISPOSABLES);
            IdentifyAll(OBJECT_SELF, OBJECT_SELF);
        }

        else if (d3() == 2)
        {
            DoTreasureGeneration( OBJECT_SELF, OBJECT_SELF, MEDIUM, GOLD|DISPOSABLES);
            IdentifyAll(OBJECT_SELF, OBJECT_SELF);
        }

        else if (d3() == 3)
        {
            DoTreasureGeneration( OBJECT_SELF, OBJECT_SELF, HIGH, GOLD|DISPOSABLES);
            IdentifyAll(OBJECT_SELF, OBJECT_SELF);
        }
    }
}
