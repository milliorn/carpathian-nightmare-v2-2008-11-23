...
* 2d8, 2d10 or 2d12 Sonic damage (LevelMod 4,5,6)
* +LevelModifier Reflex Saves
...
void ThunderWeapon(int nSlot, int nAB, int nLevelMod,  int nRounds, int bAddClang = FALSE) {
...
void FearPulse(object oBarb)
{
   ...
   int nDuration = (nBarbLvl/10);
   float fRadius = RADIUS_SIZE_LARGE + nLevelMod/2;
   effect eLink;

   int nSpeak = d10();
   if      (nSpeak==1) nSpeak = VOICE_CHAT_ATTACK;
   else if (nSpeak==2) nSpeak = VOICE_CHAT_BATTLECRY1;
   else if (nSpeak==3) nSpeak = VOICE_CHAT_BATTLECRY2;
   else if (nSpeak==4) nSpeak = VOICE_CHAT_BATTLECRY3;
   else if (nSpeak==5) nSpeak = VOICE_CHAT_CHEER;
   else if (nSpeak==7) nSpeak = VOICE_CHAT_CUSS;
   else if (nSpeak==8) nSpeak = VOICE_CHAT_ENEMIES;
   else if (nSpeak==9) nSpeak = VOICE_CHAT_TAUNT;
   else if (nSpeak==10) nSpeak = VOICE_CHAT_THREATEN;
         ...
         if (!MySavingThrow(SAVING_THROW_WILL, oTarget, nDC, SAVING_THROW_TYPE_FEAR)) {
            ...
            if (nHDTarget < nHDBarb || nHDTarget==nBarbLvl) { // Hit dice below barb.... run like hell!
               ...
               if (d100()< nLevelMod && !GetIsImmune(oTarget, IMMUNITY_TYPE_PARALYSIS)) {
                  ...
                  DestroyObject(oPee, RoundsToSeconds(nDuration));
...
void main() {
   if (!GetHasFeatEffect(FEAT_BARBARIAN_RAGE)) {
      ...
      if (GetHasFeat(FEAT_EPIC_THUNDERING_RAGE, OBJECT_SELF)) { // ADD REFLEX SAVE & SONIC DAMAGE IN IF THUNDERING
         ...
         nAttack = 5;
         if (nLevel==40) nAttack = 6;
         ThunderWeapon(INVENTORY_SLOT_RIGHTHAND, nAttack, nLevelMod, nDuration, TRUE);
         ThunderWeapon(INVENTORY_SLOT_LEFTHAND,  nAttack, nLevelMod, nDuration, FALSE);
      }
