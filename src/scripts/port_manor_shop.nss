...
//    SetIsDestroyable(TRUE, FALSE, FALSE);
    ...
    int iMaxLvl = iKillerLvl + 10;
    int iMinLvl = iKillerLvl - 10;
    ...
    while (GetIsObjectValid(oPartyMember) == TRUE) {
         ...
         if (GetArea(oPartyMember) == oArea)
         {
            ...
            // not within 10 levels of Killer - no Reward
             if ((iLevel < iMinLvl) || (iLevel > iMaxLvl)){
                ...
                DelayCommand(2.5, SendMessageToPC(oPartyMember, "You are not within 10 levels of Boss Killer - No Boss Reward"));
                DelayCommand(5.0,FloatingTextStringOnCreature("Not within 10 levels of Boss Killer", oPartyMember));
            }
            ...
            else if (GetPersistentInt(oPartyMember, sBoss, "BOSS"))
            {
                ...
                SendMessageToAllDMs("PLAYER: "+sName+" CD KEY: "+sKey+" ACCT: "+sAcct+" has been ported out for Boss Camping!!!");
            }
            // passed all tests - now give the Reward already - first set the Boss Tag...
            else {

                SetPersistentInt(oPartyMember, sBoss, TRUE, 1, "BOSS");
                DelayCommand(3600.0, DeletePersistentVariable(oPartyMember, sBoss, "BOSS"));
                ...
                CreateItemOnObject("xpbankkey", oPartyMember);
                CreateItemOnObject("xpbankkey", oPartyMember);
                CreateItemOnObject("dm_ftoken", oPartyMember);
                // if it's the Killer (and not previously disqualified) give Killer a gem
                if (oPartyMember == oKiller)
                    GenerateSockets(oKiller);
                    SendMessageToAllDMs(sName+" slayed " + sBoss +".  GP reward is: "
                    + IntToString(iRewardGP)+ " XP reward is: "
                    + IntToString(iRewardXP));
            }
        }
        else
        {
            ...
            DelayCommand(3.0,FloatingTextStringOnCreature("Must be in same area as the Boss to receive reward!", oPartyMember));
        }
