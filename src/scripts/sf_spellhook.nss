...
void main()
{
    ...
    if (nID == SPELL_ACID_FOG ||
        ...
        nID == SPELL_WEB)
    {
        oTarget = GetFirstObjectInShape(SHAPE_SPHERE,35.0,lLocal,FALSE,OBJECT_TYPE_AREA_OF_EFFECT);
        ...
        while (GetIsObjectValid(oTarget))
        {
            if(GetObjectType(oTarget) == OBJECT_TYPE_AREA_OF_EFFECT)
            {
                ...
                oTarget =GetNextObjectInShape(SHAPE_SPHERE,35.0,lLocal,FALSE,OBJECT_TYPE_AREA_OF_EFFECT);
        }
            return;
    }
    ...
    {
        SetModuleOverrideSpellScriptFinished();
        FloatingTextStringOnCreature("No SPAMMING SPELLS allowed!!  Wait a full"+
        "round(6 seconds) to cast same spell twice!!", oCaster);
        return;
    }
    ...
    else if (SummonCheck(oCaster, nID) && !GetLocalInt(oCaster, "SUMMONSPAM"))
    {
        ...
        DelayCommand(TurnsToSeconds(1), SetLocalInt(oCaster, "SUMMONSPAM", FALSE));
    }

    SetLocalInt(oCaster, sSpell, nID);
    DelayCommand(6.0, DeleteLocalInt(oCaster, sSpell));
}
