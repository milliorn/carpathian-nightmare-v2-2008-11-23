...
default: SendMessageToAllDMs("script 'spellbook_acn' has a problem"); break;
     }
    oSpell = CreateItemOnObject(sSpell, oPC);
      if (!GetIsObjectValid(oSpell))
        {
        SendMessageToAllDMs("Invalid Spell = "+sSpell);
        }
    iCount++;
    }
  DestroyObject(GetItemActivated());
break;
} // End case X2_ITEM_EVENT_ACTIVATE
} // End switch
}
