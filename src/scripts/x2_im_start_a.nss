...
void main()
{
    ...
    // Seta variavel persistente identificando que o jogador estava fazendo Crafting.
    SetPersistentInt(oPC, "WAS_CRAFTING", TRUE, 0, "Hermes");
    string sObjectTag = GetTag(oItem);
    SetPersistentString(oPC, "CRAFT_OBJECT", sObjectTag, 0, "Hermes");

    string sPlayer = GetName(oPC, FALSE),
           sItem = GetName(oItem, FALSE),
           sItem1 = GetName(oItem, TRUE);


    SendMessageToAllDMs(sPlayer+" is crafting <cσσσ>"+sItem+
    "</c> Real Name:<cσσσ> "+sItem1);
}
