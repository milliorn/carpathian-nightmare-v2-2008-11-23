...
// New Scribe Scroll constants for my revisions. -- Krit
const string  TK_SCROLL_2DA = "tk_craft_scroll";    // The Krit's scroll .2da file.
const string  TK_SCROLL_TAG = "TK_SCRIBED_SCROLL_0";// The beginning of a scribed scroll's tag.
...
// This function might no longer be used, but I'll leave it alone,
// just in case -- TK
...
// -----------------------------------------------------------------------------
// Wrapper for the crafting cost calculation, returns GP required
// Adapted for my scroll crafting. -- Krit
//
// nInnateLevel is the innate level of the spell.
// nMod is a multplier, presumably one of the constants defined in the beginning
// of this file.
// -----------------------------------------------------------------------------
int TK_CIGetCraftGPCost(int nInnateLevel, int nMod)
{
    int nCasterLevel = GetCasterLevel(OBJECT_SELF);
    int nSpellID = GetSpellId();

    // Find the .2da columns corresponding to the caster level.
    int nRange = StringToInt(Get2DAString(TK_SCROLL_2DA, "NumProps", nSpellID));
    int nIPLevel = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(nRange), nSpellID));
    // Loop until we find valid data.
    while ( nRange > 1  &&  nIPLevel == 0 )
        nIPLevel = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(--nRange), nSpellID));
    // Loop until we find the right range.
    while ( nRange > 1  &&  nIPLevel > nCasterLevel )
        nIPLevel = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(--nRange), nSpellID));

    // -------------------------------------------------------------------------
    // in case we don't get a valid IPLevel, use spell level instead
    // -------------------------------------------------------------------------
    if ( nIPLevel == 0 )
        nIPLevel = nInnateLevel;

    // The actual calculation: Effective caster level * innate level * multiplier.
    return nIPLevel * nInnateLevel * nMod;
}
...
// Looks through oHolder's inventory for an item created from blueprint sResRef
// with the tag sTag.
// If found, attempts to increase the stack size by 1.
// If successful, returns the increased stack.
// If failed, looks for another such item.
// If no increasable stack is found, returns OBJECT_INVALID.
object TK_IncrementPartialStack(object oHolder, string sResRef, string sTag)
{
    // Loop through oHolder's inventory.
    object oItem = GetFirstItemInInventory(oHolder);
    while ( GetIsObjectValid(oItem) )
    {
        // Look for the right blueprint and tag.
        if ( GetResRef(oItem) == sResRef  &&  GetTag(oItem) == sTag )
        {
            // Try to increase the stack size.
            int nSize = GetItemStackSize(oItem);
            SetItemStackSize(oItem, nSize + 1);

            // See if successful.
            if ( GetItemStackSize(oItem) == nSize + 1 )
                return oItem;
        }
        oItem = GetNextItemInInventory(oHolder);
    }

    // Failed to find a partial stack.
    return OBJECT_INVALID;
}


// This function handles the actual creating of scribed scrolls.
// The only check this makes is for optional material components.
//
// If successful, a scroll of spell nSpellID is created on oCreator, then
// returned by this function.
// On failure, no scroll is created and OBJECT_INVALID is returned.
//
// Rewritten so we can more closely match caster level.
// -- TK 4/15/07
//
// Oddity: there is no scroll for War Cry. -- Krit
object CICraftScribeScroll(object oCreator, int nSpellID)
{
    object oScroll = OBJECT_INVALID;
    int nCasterLevel = GetCasterLevel(oCreator);
    string sResRef = Get2DAString(TK_SCROLL_2DA, "BaseScroll", nSpellID);
    int nItemPropSpell = -1;
    int nItemPropClass = -1;


    // CHECK FOR POSSIBLE ABORT:

    // Find the .2da columns corresponding to the caster level.
    int nRange = StringToInt(Get2DAString(TK_SCROLL_2DA, "NumProps", nSpellID));
    int nLevelMin = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(nRange), nSpellID));
    // Loop until we find valid data.
    while ( nRange > 1  &&  nLevelMin == 0 )
        nLevelMin = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(--nRange), nSpellID));
    // Loop until we find the right range.
    while ( nRange > 1  &&  nLevelMin > nCasterLevel )
        nLevelMin = StringToInt(Get2DAString(TK_SCROLL_2DA, "Level" + IntToString(--nRange), nSpellID));
    // Convert the effective caster level to a string for later uses.
    string sLevel = IntToString(nLevelMin);

    // Retrieve the corresponding item property index.
    nItemPropSpell = StringToInt(Get2DAString(TK_SCROLL_2DA, "IProp" + IntToString(nRange), nSpellID));

    // Check for invalid data.
    if ( ( nItemPropSpell == 0  &&  nSpellID != 0 )  ||  sResRef == "" )
    {
        // Log the problem and abort.
        WriteTimestampedLogEntry(
            "x2_inc_craft::CICraftScribeScroll failed - SpellID: " + IntToString (nSpellID) +
            "; blueprint: " + sResRef + "; level range: " + IntToString(nRange) +
            "; ItemProp: " + IntToString(nItemPropSpell) + ".");
        return OBJECT_INVALID;
    }
    ...
    // (Currently, Harm, Time Stop, and True Seeing require dragon's blood.)
    string sMat = Get2DAString(TK_SCROLL_2DA, "MatComp", nSpellID);
    if ( sMat != "" )
    {
        // See if oCreator has the required component.
        object oMat = GetItemPossessedBy(oCreator, sMat);
        if ( oMat == OBJECT_INVALID )
        {
            // No component, so inform oCreator and abort.
            FloatingTextStrRefOnCreature(83374, oCreator);
            return OBJECT_INVALID;
        }

        // Consume the material component.
        DestroyObject(oMat);
    }
    ...
    string sTag = TK_SCROLL_TAG + sResRef;

    // Check for a stack that can be increased in size (and increment it).
    // (The tag is unique for each class/spell/level combo.)
    //oScroll = TK_IncrementPartialStack(oCreator, sResRef, sTag);

    //if ( oScroll == OBJECT_INVALID )
    //{
        // Create a scroll with the correct description, but a new tag.
        // (The new tag prevents bad stacking before we change item properties.)
        oScroll = CreateItemOnObject(sResRef, oCreator, 1, sTag);
        object oBonusScroll = CreateItemOnObject(sResRef, oCreator, 1, sTag);
        object oSecondBonusScroll = CreateItemOnObject(sResRef, oCreator, 1, sTag);
        // Check for a scroll creation error.
        if ( oScroll == OBJECT_INVALID )
        {
        ...
        // Rename the scroll.
        SetName(oScroll, "<cåÌH>"+GetName(oScroll, TRUE) + "</c>");
        SetName(oBonusScroll, "<cåÌH>"+GetName(oScroll, TRUE) + "</c>");
        SetName(oSecondBonusScroll, "<cåÌH>"+GetName(oScroll, TRUE) + "</c>");

    //}//if ( oScroll == OBJECT_INVALID ) (no partial stack found)
...
int CICraftCheckScribeScroll(object oSpellTarget, object oCaster)
{
    ...
    // -------------------------------------------------------------------------
    int  nLevel    = CIGetSpellInnateLevel(nID,TRUE);
    int nCost = TK_CIGetCraftGPCost(nLevel, X2_CI_SCRIBESCROLL_COSTMODIFIER);
